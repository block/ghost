...
Each package ships its own `src/skill-bundle/` with `SKILL.md` + `references/`. Each tool's CLI carries its own emit verb (`ghost map emit skill`, `ghost fingerprint emit skill`, etc.). No top-level `ghost emit skill --all`.

Cross-recipe references happen through verb names in prose ("then run `ghost drift ack`"). No hard coupling between bundles.

The `discover.md` and `generate.md` recipes are **dropped from scope** — not migrated, not re-homed. Today's fingerprint skill bundle ships them; the new ones don't.
