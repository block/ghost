...
`ack` / `track` / `diverge` are already a coherent trio, three verbs with clear meanings. Grouping under `ghost evolve <stance>` would cost a migration for no readability win. Skip unless user feedback says otherwise.
...
| Hidden references | Grep `ghost-cli` and `ghost profile|compare|review|comply|verify|scan|diff|expr-diff|fleet|context|emit|generate|ack|track|diverge|lint|viz|discover` across the repo before each phase. |
