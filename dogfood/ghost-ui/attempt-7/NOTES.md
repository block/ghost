...
`ghost-drift compare dogfood/ghost-ui/attempt-6/fingerprint.md dogfood/ghost-ui/attempt-7/fingerprint.md`:

```text
Overall Distance: 0.0%
```

All scored dimensions are unchanged: palette, spacing, typography, and surfaces remain aligned. Decisions are present in both fingerprints but embeddings are missing, so drift reports them as unscored.
