...
// Writes a survey.json with empty IDs (fix-ids will populate).
...
writeFileSync(
  "/tmp/ghost-ui-scan/survey.json",
  `${JSON.stringify(survey, null, 2)}\n`,
);
console.error(
  `\nsurvey totals: values=${valueRows.length} tokens=${tokenRows.length} components=${componentRows.length} libraries=${libraryRows.length}`,
);
