...
**Orientation files**, in reading order: `README.md` (one paragraph + use), `expression.md` (the design decisions), `registry.json` (the canonical list of components — note the `meta.expression` and `meta.expression_dimensions` extensions), `src/styles/main.css` (every token in one file), `src/lib/theme-presets.ts` (proves what is themeable), `components.json` (shadcn config, declares `style: ghost` and the path aliases).
