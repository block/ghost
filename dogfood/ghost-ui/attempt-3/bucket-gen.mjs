...
const PRESETS_TS = `${TARGET_ROOT}/src/lib/theme-presets.ts`;
...
function classify(name, value, resolved) {
...
for (const [name, slot] of byName) {
  ...
  if (aliasMatch) {
    ...
    while (true) {
      ...
      if (!nextSlot || !nextSlot.default) break;
...
const slugCount = components.reduce(
  (m, c) => ((m[c._slug] = (m[c._slug] || 0) + 1), m),
  {},
);
...
const outPath =
  "/Users/nahiyan/Development/ghost/dogfood/ghost-ui/attempt-3/bucket.json";
writeFileSync(outPath, JSON.stringify(bucket, null, 2));
