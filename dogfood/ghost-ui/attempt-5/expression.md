...
id: ghost-ui
source: llm
timestamp: 2026-05-01T18:04:43Z
sources:
  - block/ghost@packages/ghost-ui@83c2b64
observation:
  personality:
    - editorial
    - monochrome-default
...
typography:
  families:
    ...
    - serif
  sizeRamp: [10, 11, 12, 14, 16, 18, 20, 24, 28, 40, 44, 48, 64, 96, 192]
  weightDistribution:
    "300": 1
    "400": 5
    "500": 38
...
`ghost-ui` reads as a compact shadcn-derived component system wearing an editorial, monochrome-first skin. Its default theme is nearly achromatic and high-contrast, but the package also ships five runtime presets that can replace the color and radius vocabulary without changing component code. The component layer is dense and practical: small type, tight controls, pill-forward interactive elements, tokenized surfaces, and structural motion for overlays, navigation, loading, and AI-status affordances.
...
**Evidence:**
- `src/styles/main.css` declares 240 unique CSS variable names and 323 total declarations.
...
**Evidence:**
- Radius rows: `999px` (46 occurrences), `20px` (114), `18px` (49), `16px` (27), `10px` (16), `14px` (11), `24px` (10), and preset-driven `0px` (23).
...
Spacing is now visibly Tailwind-consumed rather than only token-declared. The dominant rhythm is a 4px utility grid with half-step exceptions, plus named component heights and a few large layout widths.
