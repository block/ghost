...
function buildPromptMd(fingerprint: Expression, name: string): string {
  const parts: string[] = [];
  parts.push(
...
function buildReadmeMd(fingerprint: Expression, name: string): string {
  const traits = fingerprint.observation?.distinctiveTraits ?? [];
...
function buildSkillDescription(fingerprint: Expression, name: string): string {
  const traits = fingerprint.observation?.distinctiveTraits ?? [];
...
function defaultSkillName(fingerprint?: Expression): string {
