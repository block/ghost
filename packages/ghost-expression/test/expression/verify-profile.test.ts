...
describe("verifyProfile", () => {
  ...
  it("errors when the expression invents a palette color", () => {
    const report = verifyProfile(
      expression({ brand: "#1c1c1c", neutralSteps: ["#ffffff", "#1a1a1a"] }),
      makeSurvey(),
    );
  ...
  it("warns when a same-role expression palette entry disagrees with a high-salience token", () => {
    const report = verifyProfile(
      expression({ brand: "#ffffff" }),
  ...
  it("passes with empty checks", () => {
    const report = verifyProfile(expression({ checks: "[]" }), makeSurvey());
  ...
  it("errors when shadow posture disagrees with survey shadow evidence", () => {
    ...
    const report = verifyProfile(expression(), survey);
  ...
  it("warns when a high-salience survey value is omitted from the expression", () => {
    const survey = makeSurvey();
    survey.values.push({
  ...
  it("errors when a promoted check count does not match scoped regex hits", async () => {
    ...
    const report = verifyProfile(
      expression({ checks: checkYaml({ observedCount: 0 }) }),
      makeSurvey(),
      { root: dir },
    );
  ...
  it("handles invalid regex patterns as clear verifier errors", () => {
    const report = verifyProfile(
      expression({ checks: checkYaml({ pattern: "[" }) }),
      makeSurvey(),
    );
  ...
  it("catches the Goose2 first-pass palette regression fixture", async () => {
    ...
    const expressionRaw = await readFile(
      join(
        fixtureDir,
        "..",
        "fixtures",
