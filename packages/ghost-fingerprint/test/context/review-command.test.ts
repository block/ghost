...
import { loadFingerprint } from "../../src/core/index.js";

const GHOST_UI_FINGERPRINT = fileURLToPath(
  new URL("../fixtures/ghost-ui-fingerprint/fingerprint.md", import.meta.url),
);

describe("emitReviewCommand", () => {
  it("produces a stable artifact for the ghost-ui fingerprint", async () => {
    const parsed = await loadFingerprint(GHOST_UI_FINGERPRINT, {
  ...
  it("discovers the spike output ran against a real fingerprint", async () => {
    ...
    const raw = await readFile(GHOST_UI_FINGERPRINT, "utf-8");
    expect(raw).toMatch(/^---/);
  });
});
...
function withChecks(checks: Fingerprint["checks"]): Fingerprint {
  const fp = minimalFingerprint();
  fp.checks = checks;
  return fp;
}
...
function minimalFingerprint(): Fingerprint {
  return {
    id: "minimal",
    source: "llm",
