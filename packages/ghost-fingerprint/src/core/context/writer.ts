...
export interface WriteContextOptions {
  ...
  readme?: boolean;
  /** Emit only prompt.md (skips SKILL.md / profile.md / tokens.css). Default: false. */
  promptOnly?: boolean;
  ...
  /** Source path (e.g. ".ghost/fingerprint/profile.md") — surfaced in generated file headers. */
  sourcePath?: string;
...
export async function writeContextBundle(
  ...
  const exprPath = join(options.outDir, "profile.md");
...
export function buildSkillMd(
  ...
  const fileList = [
    "- `profile.md` — non-enforcing design-language prior (YAML digest + Character/Signature/References/Decisions)",
    "- `prompt.md` — generation prompt distilled from profile.md",
    ...(includesCss
      ? [
...
Read \`profile.md\` first — it is the design-language prior. It has these layered sections:
...
5. **Checks** — active gates from \`.ghost/fingerprint/checks.yml\` when available
...
Generated by \`ghost-fingerprint emit context-bundle\`. Grounding material for AI UI generation in the **${name}** design language.${personalityLine}

## Files

- \`SKILL.md\` — Agent Skill manifest (user-invocable)
- \`profile.md\` — non-enforcing design-language prior (YAML frontmatter + Character/Signature/Decisions)
- \`prompt.md\` — portable prompt distilled from the profile
- \`tokens.css\` — CSS custom properties derived from fingerprint tokens
- \`README.md\` — this file
...
**As context for any LLM:** load \`profile.md\` into the system prompt. For more explicit grounding, concatenate with \`tokens.css\`.

**Feedback loop:** ask your host agent to review the generated output against this profile and run \`ghost-drift check\` in the source repo when a full package is available.
`;
}
...
function buildSkillDescription(fingerprint: Fingerprint, name: string): string {
  ...
  const traitPhrase = personality.length
    ...
    : "";
  return `Use this skill to generate UI in the ${name} design language${traitPhrase}. Contains the profile prior and token reference.`;
}
