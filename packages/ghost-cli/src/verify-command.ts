...
export function registerVerifyCommand(cli: CAC): void {
  cli
    .command(
      "verify [expression]",
      "Run generate→review over the standard prompt suite and aggregate drift",
    )
    .option(
      "--suite <path>",
    ...
    .option(
      "-o, --out <file>",
      "Write JSON report to file (default: stdout CLI summary)",
    )
    .option("--json", "Emit JSON report to stdout")
    .option("--verbose", "Show per-prompt results in CLI output")
