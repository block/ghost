...
handoffs:
  - label: Suggest minimal fixes that close the drift gap
    skill: remediate
    prompt: Given the drift findings, suggest the minimal code changes that bring the diff back inside the fingerprint
  ...
  - label: Track a new fingerprint
    command: ghost-drift track
    prompt: Track the provided fingerprint.md as the new reference
---
...
### 1. Read the fingerprint
    ...
    ghost-fingerprint describe fingerprint.md

This prints a section map — frontmatter range, body sections (`# Character`, `# Signature`, `# Decisions`), and each `### dimension` block under Decisions, with line ranges and token estimates. Use it to plan what to load.
...
- **Surface drift:** `border-radius` not in `surfaces.borderRadii`, `box-shadow` present when `surfaces.shadowComplexity: deliberate-none`, or absent when the fingerprint expects shadows.
...
- `ghost-drift track <fingerprint.md>` — "track another fingerprint as the reference."
