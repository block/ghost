...
export async function resolveTrackedFingerprint(
  ...
  switch (target.type) {
    ...
    default:
      throw new Error(
        `Cannot resolve tracked fingerprint from target type "${target.type}". Generate one first by running the fingerprint recipe in your host agent (install with "ghost skill install").`,
      );
  }
}
