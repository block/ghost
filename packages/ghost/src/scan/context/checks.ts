import type { Check, DriftSeverity, Fingerprint } from "#ghost-core";
import {
...
export function resolveFingerprintChecks(fp: Fingerprint): ResolvedCheck[] {
 ...
 * structured frontmatter fields. v0 fingerprints don't carry the survey
 * directly; the proxy keeps presence-floor escalation deterministic until
 * the check author supplies `observed_count`.
 */
export function surveyCountForCheck(check: Check, fp: Fingerprint): number {
