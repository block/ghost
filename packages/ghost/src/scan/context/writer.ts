...
function buildPromptMd(fingerprint: Fingerprint, name: string): string {
  ...
  if (checks.length) {
    ...
    parts.push(
      "# Checks\n\nNo package checks were embedded in this context bundle. Treat this as generation guidance; run `ghost check` in the source repo for active gates.",
    );
  }
...
Generated by \`ghost emit context-bundle\`. Grounding material for AI UI generation in the **${name}** design language.${personalityLine}
...
**Feedback loop:** ask your host agent to review the generated output against this fingerprint and run \`ghost check\` in the source repo when a full package is available.
`;
}
