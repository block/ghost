...
function calibrationFooter(fp: Fingerprint, resolved: ResolvedCheck[]): string {
  ...
  lines.push(
    ...
    "",
    `Generated from a fingerprint object (${(fp.checks ?? []).length} legacy checks). Re-run \`ghost emit review-command\` after package updates.`,
  );
  return lines.join("\n");
}
...
This fingerprint has no embedded checks, so this command uses a coarse token fallback from palette, spacing, typography, and surfaces. Treat findings as lower-confidence than \`ghost check\`; promote enforceable rules in \`.ghost/checks.yml\` when a pattern should become a gate.`;
}
