...
2. Separate check failures from advisory findings.
3. For check failures, patch the smallest implementation change that
   satisfies the check and preserves product intent.
...
Use `ghost review` after implementation changes. Use `ghost validate` after fingerprint edits.
