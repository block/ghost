...
3. For advisory review, run `ghost checks --diff <patch>` to route the diff to
   its surfaces' checks with grounding.
4. For generation setup, run `ghost gather --path <file> --format json` when a
   target path is known. For prompt-shaped work, match the ask to a surface via
   the menu (`ghost gather`) and run `ghost gather <surface> --format json`.
