...
.ghost/
  ...
  glossary.md         # the author's category vocabulary
...
- `materials` is one list of locators for the concrete stuff the truth is about:
  repo-relative paths/globs or absolute HTTPS URLs. Guidance stays in prose.
...
directly: it emits the same prose plus materials and appends structured events to
`.ghost/.events` for local tuning.
