...
Selective context should also reduce cost, but there is no published evaluation yet. The goal is not context minimalism. Some tasks need many nodes, and a serious package needs enough depth to change the work. Gather lets the agent take what the task requires without pretending the whole brand belongs in one prompt.
