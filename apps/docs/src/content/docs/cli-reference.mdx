...
| `2` | The command was called wrong: a bad flag or argument, a missing package for a command that needs one (`gather`), a `pull` with no known ids, or `review` without the checks haunt installed. |
...
the fingerprint only; pass `--with checks` to also add the checks haunt in one
move.
...
under `.ghost/haunts/<id>/` anchored by a thin `haunt.yml`. Haunts are
feed-back only — they never leak into generation context, and they are never
emitted by `gather` or `pull`. The first haunt is `checks`: review assertions
consumed by `ghost review`.

<CliHelp tool="ghost" command="haunt" hideDescription />

```bash
ghost haunt add checks
ghost haunt list
ghost haunt remove checks
```

`haunt list` shows what is installed and what is available. `haunt remove`
deletes the haunt directory; like every fingerprint edit, ordinary Git review
...
installed haunts, check references, and that each node's kind prefix is
...
`review` requires the checks haunt (`ghost haunt add checks`). Without it, the
...
Each check is one markdown file under `.ghost/haunts/checks/`:
