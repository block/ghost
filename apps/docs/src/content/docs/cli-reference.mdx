...
When you do not know whether what you need is a node, a surface, or a check,
search across all of them at once. Results are ranked: a verbatim match wins
(an exact id, then a name, description, or body substring), then a whole-name
typo fallback, then multi-word queries by how many of their words a candidate
covers — so a natural phrase like `payment confirmation screen` still finds the
right surface. Each result carries the follow-up command to run. Restrict with
`--type <node|surface|check>` and cap with `--limit`.

<CliHelp tool="ghost" command="search" hideDescription />

```bash
ghost search checkout
ghost search trust --type node
ghost search contrast --type check
ghost search marketing --format json
```
