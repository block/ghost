...
because each pull also appends one line (a timestamp plus the pulled ids) to
`.ghost/.pulls`.

That file is a local history tape for the fingerprint's author: tweak a node's
description, re-run the task, and `tail .ghost/.pulls` shows whether the agent
reached for the truth. It is a dotfile (never a node), gitignored by the
scaffold, and safe to delete at any time. Ghost never reads it back — nothing
selects, ranks, or validates from it.
...
An unknown id exits `2` with a closest-id hint and appends nothing.
