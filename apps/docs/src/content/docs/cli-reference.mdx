...
<DocSection title="Export The Fingerprint">

### Package a portable artifact: `export`

`export` packages the fingerprint as a portable brand artifact: a tarball of
...
`export` refuses to package a fingerprint with validation errors. Run
`ghost validate` and fix them first.

<CliHelp tool="ghost" command="export" hideDescription />

```bash
ghost export                    # write <id>-fingerprint.tgz with an audit summary
