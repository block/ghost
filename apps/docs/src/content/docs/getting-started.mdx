...
| Tool | Job | Verbs |
| --- | --- | --- |
| `ghost-fingerprint` | Create and check the root `.ghost/` bundle. | `inventory`, `scan-status`, `lint`, `verify`, `describe`, `diff`, `survey <op>`, `emit` |
...
| `ghost-memory` | Activate and capture optional product-experience memory. | `emit skill` |
...
Use `ghost-memory` when work reveals a decision Ghost should remember:
...
`ghost-memory` recipes write proposals first. Humans promote durable memory into
