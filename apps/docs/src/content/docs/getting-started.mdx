...
| Surface | Job | Verbs |
| --- | --- | --- |
| `ghost` | Create and check the root `.ghost/` bundle, review diffs, compare bundles, and record drift intent. | `init`, `scan`, `inventory`, `lint`, `verify`, `describe`, `diff`, `survey <op>`, `check`, `review`, `compare`, `ack`, `track`, `diverge`, `emit`, `skill install` |
...
`ghost review` emits advisory context grounded in `.ghost/patterns.yml`,
