...
compose context, run checks, and review changes end to end.

</Callout>

Ghost gives your agent the composition behind a product surface — what it's
for, what it's made of, and the patterns that make it feel intentional —
**before** it builds, and checks the result after it changes. The public package
is `@anarchitecture/ghost`, and it installs one CLI: `ghost`.
...
`relates`, plus free-form passthrough keys. Raw repo observations
are optional authoring evidence. Curate durable intent, inventory, and composition into the
