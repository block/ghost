...
interviewing, reading repo evidence, drafting node prose, and asking you to
curate the claims. `ghost init` scaffolds `manifest.yml`, a starter `glossary.md`,
...
decides when the truth applies. Reserved at the package root: `manifest.yml` and
`glossary.md`; every other `*.md` is a node.
