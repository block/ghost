...
Ghost is **feed-forward**: it grounds generation. It does not review or grade
output. You work through a short loop:
...
ghost gather      # emit the menu; your agent selects the truths the task needs
...
file. That one is for you: tweak a node's description, re-run the task, and
`tail .ghost/.pulls` shows whether the agent reached for the truth. Ghost never
reads it back.
...
`glossary.md`, and the `haunt/` subtree; every other `*.md` is a node.

Write and read every prose node through three lenses: intent, inventory, and
composition. `ghost gather` emits the menu; the agent selects and manifests.
