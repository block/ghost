...
Repo-bound reality — components, tokens, paths, building blocks — belongs to
the haunt plugin's inventory, not fingerprint prose. What a codebase does is
not what a brand means; what it repeats may be legacy, not stance.
...
3. **Draft** - write the smallest useful node prose from what the human said
   or showed, reading each node through the intent, inventory, and composition
   lenses.
