...
| `relates` | Lateral links to other nodes by path id (`to: core/trust`). |
...
distinct product owns its own fingerprint — a separate app in a monorepo —
and target it with `ghost --package <dir>`.
