---
title: Authoring
description: Co-author ghost brand guidance through elicitation, human intent, shown material, agent synthesis, and Git review.
kicker: Docs
section: guide
order: 20
slug: authoring
...
A ghost package is not a scan dump. It is durable brand guidance that a
human and agent shape together — through **elicitation, not scanning**.

The human names the intent: what the brand should feel like, who it serves,
which guidance is universal, which guidance holds only under a condition, and what should
...
agent synthesizes drafts, but ordinary Git review is where ghost package edits
become canonical.
...
1. **Interview** - ask what the brand should feel like, what it should never
   ...
   usually yields a sharper decision than an hour of affirmative description.
2. **Work the material** - ask for what the human can show: screenshots,
   ...
   why it was rejected. Treat every artifact as evidence to curate, not guidance
   to copy.
3. **Draft** - write the smallest useful node prose from what the human said
   or showed, asking of each node why, with what, and how it is assembled.
...
not establish brand guidance by themselves — the human ratifies what is
intentional.
...
A `.ghost/` package is a **flat set of prose nodes** — no hierarchy, no inheritance,
...
State guidance at the altitude where it applies. Universal guidance is stated
plainly. Narrower guidance names its **condition** in the prose — the *situation*
where it applies ("when a surface must show many items at once, carry hierarchy with
weight, not color"), never a destination or filing bucket ("for dashboards:").
The agent reads the condition and decides when the guidance applies; that
...
While drafting a node's prose body, ask three questions of every node: *why*
...
Useful brand guidance should help future agents choose, restrain, anchor, and
...
- State the condition under which a narrower guidance applies.
...
One contract per package: a repo's `.ghost/` is the whole ghost package. Reach for
a second package only when a distinct product owns its own ghost package — a
...
Uncommitted or unmerged ghost package edits are drafts. Checked-in ghost node prose
is canonical.
