...
description: Opt in to review assertions with the checks directory, bind them to nodes, and assemble advisory review packets from a diff.
kicker: Docs
section: guide
order: 25
...
change exists, they help the same agent review it against the guidance you wrote
down.
...
brand guidance, and do not invent obligations the package never states.
...
package's author, not errors.

- **Unmatched files** — the diff touched something no node claims. Maybe that
  surface deserves guidance, or maybe it is genuinely out of scope.
- **Unguarded materials** — a node points at concrete material, but no check
  guards it. The node feeds generation but nothing asserts it survives review.
