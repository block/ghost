...
description: Opt in to review assertions with the checks haunt, bind them to fingerprint nodes, and assemble advisory review packets from a diff.
kicker: Docs
section: guide
order: 25
...
<DocSection title="Haunts: Opt-In Capabilities">
...
thin `haunt.yml` (`schema: ghost.haunt/v1` + `id`). Two properties keep haunts
honest:

- **Opt-in.** Core `ghost init` ships no haunts. You add them explicitly.
- **Feed-back only.** Nothing under `haunts/` is a node. `ghost gather` and
  `ghost pull` never emit haunt content, so haunts cannot leak into generation
  context.

```bash
ghost haunt add checks     # attach the checks haunt
ghost haunt list           # Haunting this fingerprint: checks
ghost haunt remove checks  # detach it again
ghost init --with checks   # or opt in at scaffold time
```

Checks are the first haunt. `ghost validate` validates installed haunts along
with the rest of the package, and ordinary Git review is the approval boundary
for haunt changes, same as any fingerprint edit.

</DocSection>

<DocSection title="Authoring Checks">

A check is one markdown file under `.ghost/haunts/checks/`: a review assertion
...
`ghost review` requires the checks haunt and a diff. By default it diffs the
